...
const inicializarBase = async () => {
  try {
    ...
    if (fs.existsSync(archivo)) {
      console.log('Base de datos ya existe, no se inicializa.');
...
async function DatosArticulos() {
  await articulos.bulkCreate([
    // Agrega tus datos de artículos aquí
...
async function DatosUsuarios() {
  await usuarios.bulkCreate([
    { IdUsuario: 1, Nombre: 'admin', Clave: '123', Rol: 'jefe' },
    { IdUsuario: 2, Nombre: 'juan', Clave: '123', Rol: 'empleado' },
    { IdUsuario: 3, Nombre: 'ana', Clave: 'ana123', Rol: 'empleado' },
  ]);
}



// Verificar si es el archivo principal y ejecutar
